...
window.editRecipe = function (recipeId) {
    fetch(`${BASE_URL}/${recipeId}`)
      ...
      .then(recipe => {
        ...
        let selectedCategoryNames = selectedCategories.map(
          catId => categoryMapping[catId] || "Unknown"
        );
  
        // Debugging: Check categories array
        console.log("Categories Received:", selectedCategories);
...
document.querySelectorAll("#editCategoryList .dropdown-item").forEach(item => {
  ...
  // Initialize dropdown text for selected categories
  document.getElementById("editCategoryDropdown").innerText =
