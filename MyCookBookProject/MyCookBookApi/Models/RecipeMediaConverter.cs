...
namespace MyCookBookApi.Models
{
    public class RecipeMediaConverter : IFirestoreConverter<List<RecipeMedia>>
    {
        ...
        public List<RecipeMedia> FromFirestore(object value)
        {
            ...
            {
                return list.OfType<Dictionary<string, object>>().Select(dict => new RecipeMedia
                {
                    Url = dict.ContainsKey("Url") ? dict["Url"].ToString() : "", Type = dict.ContainsKey("Type") ? dict["Type"].ToString() : "", Order = dict.ContainsKey("Order") ? Convert.ToInt32(dict["Order"]) : 0 }).ToList();
                }
