...
// _firestoreDb = FirestoreDb.Create("YourDataStoreDb");
// }
...
namespace MyCookBookApi.Repositories
{
    public class FirebaseDbRecipeRepository : IRecipeRepository
    {
        ...
        public FirebaseDbRecipeRepository()
        {
            ...
            string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "wwwroot", "YourServiceAccountKey.json");
            if (!File.Exists(path))
            {
                throw new FileNotFoundException("Firebase service account key not found.", path);
            }
            ...
            Environment.SetEnvironmentVariable("GOOGLE_APPLICATION_CREDENTIALS", path);
            _firestoreDb = FirestoreDb.Create("mycookbookdb");
